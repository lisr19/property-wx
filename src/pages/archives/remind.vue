	...
	<view  class="wrap">
		...
		<view class="items">
			...
			<view class="item" v-for="(item,index) in dataList" :key="index">
				<p class="title">档案编号：{{item.da_ph}}</p>
				<p>档案名称：{{item.da_name}}</p>
<!--				<p>合同对象：{{item.da_ph}}</p>-->
...
<script>
	...
	import {gettxList} from "@/utils/api/index"
	export default {
		...
		methods: {
			...
			async gettxList(params){
				...
				let res = await gettxList(params)
				if(res.code === 0){
